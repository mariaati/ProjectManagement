...
router.get("/getOne/:id", async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM faculties WHERE id=$1", [req.params.id]);
...
router.put("/update/:id", authMiddleware, async (req, res) => {
    try {
        const { name, description } = req.body;
        const result = await pool.query(
            "UPDATE faculties SET name=$1, description=$2, updated_at=NOW() WHERE id=$3 RETURNING *",
